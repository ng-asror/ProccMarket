...
use Klev\CryptoPayApi\CryptoPayHttpClientInterface;
...
class CustomCryptoPay
{
     ...
     * @throws CryptoPayException
     */
    public function createInvoice(CreateInvoice $createInvoice): array
    {
     ...
     * @throws CryptoPayException
     */
    public function getWebhookUpdates($rawUpdates, string $signature, bool $throwVerifyError = true):? Update
    {
        $updates = null;
        if ($rawUpdates && $this->verifyWebhookUpdates($rawUpdates, $signature, $throwVerifyError)) {
    ...
    protected function request(string $method, array $data = []): array
    {
        try {
            ...
            return [
                "result" => [
                    "success" => false,
                    "message" => $out['error'] ?? 'Unexpected response',
                    "raw" => $out,
                ]
            ];
            ...
            return [
                "result" => [
                    "success" => false,
                    "message" => $out['error']['name'] ?? $e->getMessage(),
                    "raw" => $out,
                ]
            ];
        } catch (\Exception $e) {
     ...
     * @throws CryptoPayException
     */
    public function verifyWebhookUpdates($rawUpdates, string $signature, bool $throwVerifyError = true): bool
    {
        try {
            ...
            $calcHash = hash_hmac('sha256', $rawUpdates, $secret);
